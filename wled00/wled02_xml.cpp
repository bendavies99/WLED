...
char *XML_response(AsyncWebServerRequest *request, bool includeTheme, char *dest)
{
  ...
  oappend("</vs>");
  if (request != nullptr)
    request->send(200, "text/xml", obuf);
}
...
void getSettingsJS(byte subPage, char *dest)
{
  ...
  if (subPage == 1)
  {
    ...
    for (int i = 0; i < 4; i++)
    {
      ...
      k[0] = 'I';
      sappend('v', k, staticIP[i]);
      k[0] = 'G';
      sappend('v', k, staticGateway[i]);
      k[0] = 'S';
      sappend('v', k, staticSubnet[i]);
    }
  ...
  if (subPage == 5)
  {
    ...
    for (int i = 0; i < 8; i++)
    {
      k[1] = 48 + i; //ascii 0,1,2,3
      k[0] = 'H';
      sappend('v', k, timerHours[i]);
      k[0] = 'N';
      sappend('v', k, timerMinutes[i]);
      k[0] = 'T';
      sappend('v', k, timerMacro[i]);
      k[0] = 'W';
      sappend('v', k, timerWeekday[i]);
    }
  }
