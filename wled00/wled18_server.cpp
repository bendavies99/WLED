...
void initServer()
{
  ...
  AsyncCallbackJsonWebHandler *handler = new AsyncCallbackJsonWebHandler("/json", [](AsyncWebServerRequest *request, JsonVariant &json) {
    ...
    if (root.isNull())
    {
      request->send(500, "application/json", "{\"error\":\"Parsing failed\"}");
      return;
    }
    if (deserializeState(root))
    {
      serveJson(request);
      return;
    } //if JSON contains "v" (verbose response)
...
//init ota page
    ...
    server.on("/update", HTTP_POST, [](AsyncWebServerRequest *request) {
      ...
      doReboot = true; }, [](AsyncWebServerRequest *request, String filename, size_t index, uint8_t *data, size_t len, bool final) {
      if(!index){
...
void serveIndex(AsyncWebServerRequest *request)
{
  ...
  if (uiConfiguration == 0 && request->hasHeader("User-Agent"))
    serveMobile = checkClientIsMobile(request->getHeader("User-Agent")->value());
  else if (uiConfiguration == 2)
    serveMobile = true;

#ifdef WLED_ENABLE_FS_SERVING
  if (serveMobile)
  {
    if (handleFileRead(request, "/index_mobile.htm"))
      return;
  }
  else
  {
...
String settingsProcessor(const String &var)
{
  ...
  if (var == "SCSS")
    return String(PAGE_settingsCss);
  return String();
}
...
void serveSettings(AsyncWebServerRequest *request)
{
  ...
  case 1:
    ...
    break;
  case 2:
    request->send_P(200, "text/html", PAGE_settings_leds, settingsProcessor);
    break;
  case 3:
    request->send_P(200, "text/html", PAGE_settings_ui, settingsProcessor);
    break;
  case 4:
    request->send_P(200, "text/html", PAGE_settings_sync, settingsProcessor);
    break;
  case 5:
    request->send_P(200, "text/html", PAGE_settings_time, settingsProcessor);
    break;
  case 6:
    request->send_P(200, "text/html", PAGE_settings_sec, settingsProcessor);
    break;
  case 255:
    request->send_P(200, "text/html", PAGE_welcome, settingsProcessor);
    break;
  default:
    request->send_P(200, "text/html", PAGE_settings, settingsProcessor);
  }
}
