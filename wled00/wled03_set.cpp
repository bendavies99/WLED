...
void handleSettingsSet(AsyncWebServerRequest *request, byte subPage)
{
  ...
  if (subPage == 1)
  {
    ...
    if (request->arg("CP").charAt(0) != '*')
      strcpy(clientPass, request->arg("CP").c_str());

    strcpy(cmDNS, request->arg("CM").c_str());

    int t = request->arg("AT").toInt();
    if (t > 9 && t <= 255)
      apWaitTimeSecs = t;
    strcpy(apSSID, request->arg("AS").c_str());
    apHide = request->hasArg("AH");
    int passlen = request->arg("AP").length();
    if (passlen == 0 || (passlen > 7 && request->arg("AP").charAt(0) != '*'))
      strcpy(apPass, request->arg("AP").c_str());
    t = request->arg("AC").toInt();
    if (t > 0 && t < 14)
      apChannel = t;
...
#if LEDPIN == 3
    if (ledCount > 300)
      ledCount = 300; //DMA method uses too much ram
#endif
#endif
  ...
  if (subPage == 4)
  {
    ...
    {
      strcpy(blynkApiKey, request->arg("BK").c_str());
  ...
  if (subPage == 5)
  {
    ...
    if (ntpEnabled && WiFi.status() == WL_CONNECTED && !ntpConnected)
      ntpConnected = ntpUdp.begin(ntpLocalPort);

    if (request->hasArg("OL"))
    {
      overlayDefault = request->arg("OL").toInt();
      if (overlayCurrent != overlayDefault)
        strip.unlockAll();
...
bool handleSet(AsyncWebServerRequest *request, const String &req)
{
  ...
  if (pos > 0)
  {
    ...
    pos = req.indexOf("IN");
    if (pos < 1)
      XML_response(request, false, nullptr);
    return true;
  ...
  if (pos > 0)
  {
    _setRandomColor(getNumVal(&req, pos), false);
  }
...
//set hue polling light: 0 -off
  ...
  pos = req.indexOf("RV=");
  if (pos > 0)
    strip.getSegment(0).setOption(1, req.charAt(pos + 3) != '0');

  //deactivate nightlight if target brightness is reached
  if (bri == nightlightTargetBri)
    nightlightActive = false;
  ...
  if (pos > 0)
    presetApplyCol = (req.charAt(pos + 3) != '0');
...
//cronixie
  ...
  pos = req.indexOf("IN");
  if (pos < 1)
    XML_response(request, (req.indexOf("&IT") > 0), nullptr); //include theme if firstload
