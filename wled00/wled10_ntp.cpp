...
void checkTimers()
{
  if (lastTimerMinute != minute(local)) //only check once a new minute begins
  {
    ...
    {
      if (timerMacro[i] != 0 && (timerHours[i] == hour(local) || timerHours[i] == 24) //if hour is set to 24, activate every hour
          && timerMinutes[i] == minute(local) && (timerWeekday[i] & 0x01)             //timer is enabled
