...
void loadSettingsFromEEPROM(bool first)
{
  ...
  colS[0] = EEPROM.read(246);
  col[0] = colS[0];
  colS[1] = EEPROM.read(247);
  col[1] = colS[1];
  colS[2] = EEPROM.read(248);
  col[2] = colS[2];
  briS = EEPROM.read(249);
  bri = briS;
  ...
  effectDefault = EEPROM.read(324);
  effectCurrent = effectDefault;
  effectSpeedDefault = EEPROM.read(325);
  effectSpeed = effectSpeedDefault;
  ...
  if (lastEEPROMversion < 8 && overlayDefault > 0)
    overlayDefault--; //overlay mode 1 (solid) was removed
  ...
  colS[3] = EEPROM.read(371);
  col[3] = colS[3];
  useRGBW = EEPROM.read(372);
  effectPaletteDefault = EEPROM.read(373);
  effectPalette = effectPaletteDefault;
  ...
  if (lastEEPROMversion > 1)
  {
    for (byte i = 0; i < 4; i++)
    {
      colSecS[i] = EEPROM.read(378 + i);
      colSec[i] = colSecS[i];
    }
  }
  if (lastEEPROMversion > 3)
  {
    effectIntensityDefault = EEPROM.read(326);
    effectIntensity = effectIntensityDefault;
...
//force default UI since mobile is unavailable
  ...
  if (EEPROM.read(396))
    utcOffsetSecs = -utcOffsetSecs; //negative
...
void applyMacro(byte index)
{
  ...
  if (mc.indexOf(forbidden) >= 0)
    return;
  handleSet(nullptr, mc);
}

void saveMacro(byte index, String mc, bool sing) //only commit on single save, not in settings
{
  index -= 1;
  if (index > 15)
    return;
