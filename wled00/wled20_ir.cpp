...
bool decodeIRCustom(uint32_t code)
{
  ...
  if (code != IRCUSTOM_MACRO1)
    colorUpdated(2); //don't update color again if we apply macro, it already does it
  return true;
}
...
void decodeIR(uint32_t code)
{
  ...
  if (code > 0xFFFFFF)
    ...
    decodeIR44(code); //is in 44-key remote range
  else if (code > 0xF70000 && code < 0xF80000)
...
void decodeIR24(uint32_t code)
{
  ...
  case IR24_RED:
    ...
    break;
  case IR24_REDDISH:
    colorFromUint32(COLOR_REDDISH);
    break;
  case IR24_ORANGE:
    colorFromUint32(COLOR_ORANGE);
    break;
  case IR24_YELLOWISH:
    colorFromUint32(COLOR_YELLOWISH);
    break;
  case IR24_YELLOW:
  ...
  case IR24_GREEN:
    ...
    break;
  case IR24_GREENISH:
    colorFromUint32(COLOR_GREENISH);
    break;
  case IR24_TURQUOISE:
    colorFromUint32(COLOR_TURQUOISE);
    break;
  case IR24_CYAN:
    colorFromUint32(COLOR_CYAN);
    break;
  case IR24_AQUA:
    colorFromUint32(COLOR_AQUA);
    break;
  case IR24_BLUE:
    colorFromUint32(COLOR_BLUE);
    break;
  case IR24_DEEPBLUE:
    colorFromUint32(COLOR_DEEPBLUE);
    break;
  case IR24_PURPLE:
    colorFromUint32(COLOR_PURPLE);
    break;
  case IR24_MAGENTA:
    colorFromUint32(COLOR_MAGENTA);
    break;
  case IR24_PINK:
  ...
  case IR24_FADE:
    if (!applyPreset(3))
      effectCurrent = FX_MODE_BREATH;
    break;
  case IR24_SMOOTH:
    if (!applyPreset(4))
      effectCurrent = FX_MODE_RAINBOW;
    break;
  default:
    return;
  }
