...
void reconnectHue()
{
  if (WiFi.status() != WL_CONNECTED || !huePollingEnabled)
    return;
...
void sendHuePoll()
{
  if (hueClient == nullptr || !hueClient->connected())
    return;
  String req = "";
  if (hueAuthRequired)
  {
...
void onHueData(void *arg, AsyncClient *client, void *data, size_t len)
{
  ...
  if (str[0] == '[') //is JSON array
  {
    ...
    if (error)
    {
      strcpy(hueError, "JSON parsing error");
      return;
    }
    ...
    if (hueErrorCode) //hue bridge returned error
    {
      ...
      {
      case 1:
        strcpy(hueError, "Unauthorized");
        hueAuthRequired = true;
        break;
      case 3:
        strcpy(hueError, "Invalid light ID");
        huePollingEnabled = false;
        break;
      case 101:
        strcpy(hueError, "Link button not pressed");
        hueAuthRequired = true;
        break;
      default:
        char coerr[18];
  ...
  if (hueApplyColor)
  {
    ...
    {
    case 1:
      if (hueX != hueXLast || hueY != hueYLast)
        colorXYtoRGB(hueX, hueY, col);
      hueXLast = hueX;
      hueYLast = hueY;
      break;
    case 2:
      if (hueHue != hueHueLast || hueSat != hueSatLast)
        colorHStoRGB(hueHue, hueSat, col);
      hueHueLast = hueHue;
      hueSatLast = hueSat;
      break;
    case 3:
      if (hueCt != hueCtLast)
        colorCTtoRGB(hueCt, col);
      hueCtLast = hueCt;
      break;
    }
  }
